...
Code Zero reads credentials and deployment policy exclusively from the environment. Endpoint URLs and credentials can never be named or embedded in `.code-zero.yml`, so untrusted repository policy cannot redirect a secret.
...
| `CODE_ZERO_MODEL`              | Default model name                                                    |
| `CODE_ZERO_MODEL_BASE_URL`     | Operator-owned base URL for custom gateways and self-hosted endpoints |
...
| `CODE_ZERO_CHECKOUT_PATH` | Checkout the webhook route binds incoming events to |
...
| Variable                         | Purpose                                                  |
| -------------------------------- | -------------------------------------------------------- |
| `CODE_ZERO_BUILD_COMMIT`         | Full commit SHA the bundle was built from                |
| `CODE_ZERO_BUILD_BRANCH`         | Git branch                                               |
| `CODE_ZERO_BUILD_PR_NUMBER`      | Pull request number, for a pull-request deploy           |
| `CODE_ZERO_BUILD_URL`            | URL of this deploy                                       |
| `CODE_ZERO_BUILD_PRODUCTION_URL` | URL of the production domain                             |
| `CODE_ZERO_BUILD_ENV`            | Deploy channel: `dev`, `preview`, `canary`, or `release` |

Setting any of them takes precedence over every auto-detected platform, so an operator's answer
always wins over an inferred one. `CODE_ZERO_BUILD_ENV` is the field detection can never work out
